...
export const createUser = async (userData: {
  ...
  try {
    const result = await query<User>(sql, [username, email, passwordHash]);
    ...
    // Basic error handling for unique constraint violation (e.g., username or email already exists)
    if (error.code === "23505") {
