// /workspaces/kogum/packages/frontend/src/pages/GamePage.tsx
import React, { useState, useEffect, useRef } from "react";
import CardComponent from "../components/Card";
import type { ClientCardData, PuzzleData } from "../types";

interface GamePageProps {
  // Props like initial puzzle type (e.g., 'kogum' vs 'random') can be added if behavior differs
  // For now, we assume it fetches a random puzzle on mount/navigation
  pageTitle: "køgum" | "randøm";
  key: string; // To force re-mount and re-fetch when navigating between kogum/random if needed
}

const GamePage: React.FC<GamePageProps> = ({ pageTitle }) => {
  ...
  const fetchNewPuzzle = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/puzzles/random"); // Or a different endpoint for 'today' if needed
      if (!response.ok) {
  ...
  useEffect(() => {
    fetchNewPuzzle();
  }, [pageTitle]); // Re-fetch if pageTitle changes (e.g. from kogum to random, if they were different components)
    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          ...
          <button onClick={handleNewPuzzleClick} disabled={!puzzle}>
            New Puzzle
          </button>
        </div>
      </div>
